  ...
  while (( "$#" )); do
    ...
    mailman-core|all)
      docker run --rm \
        ...
        debian:stretch-slim /bin/tar --warning='no-file-ignored' --use-compress-program="gzip --rsyncable --best" -Pcvpf /backup/backup_mailman-core.tar.gz /mailman-core
      ;;&
    mailman-db|all)
      SQLIMAGE2='mariadb:10.3'
      docker run --rm \
        --network $(docker network ls -qf name=${CMPS_PRJ}_mailcow-network) \
        -v $(docker volume ls -qf name=${CMPS_PRJ}_mailman-database-vol-1):/var/lib/mysql/:ro \
        --entrypoint= \
        -v ${BACKUP_LOCATION}/mailcowmailman3-${DATE}:/backup \
        ${SQLIMAGE2} /bin/sh -c "mysqldump -hdatabase -uroot -p${MMDBROOT} --all-databases | gzip > /backup/backup_mailman_mysql.gz"
      ;;&
    mailman-web|all)
      /bin/tar --warning='no-file-ignored' --use-compress-program="gzip --rsyncable --best" -Pcvpf ${BACKUP_LOCATION}/mailcowmailman3-${DATE}/backup_mailman-web.tar.gz ../data/mailman/web
      ;;
    esac
    shift
  done
...
if [[ ${1} == "backup" ]]; then
  ...
  if [[ -z $(find "${FOLDER_SELECTION[${input_sel}]}" -maxdepth 1 -type f,d -regex ".*\(redis\|rspamd\|mysql\|crypt\|vmail\|postfix\|\mailman-core|\mailman-db|\mailman-web\).*") ]]; then
