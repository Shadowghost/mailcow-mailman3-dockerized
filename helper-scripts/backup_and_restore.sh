...
if [[ ${1} == "backup" && ! ${2} =~ (vmail|redis|rspamd|postfix|mysql|mailman-core|mailman-db|all) ]]; then
...
if [[ ${1} == "backup" ]]; then
  ...
  if [[ -z $(find "${FOLDER_SELECTION[${input_sel}]}" -maxdepth 1 -type f -regex ".*\(redis\|rspamd\|mysql\|vmail\|postfix\|\mailman-core|\mailman-db|\mailman-web\).*") ]]; then
